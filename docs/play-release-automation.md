...
3. The existing `Build Flutter APK and App Bundle` workflow runs on the tag:
   ...
   - downloads the release debug symbols workflow artifact
...
The Play upload job does not need Android signing secrets. Signing remains in the existing release build job, which still requires the existing Android keystore secrets. The job reuses the debug symbols generated by the release build and passes them to Google Play with the upload action's `debugSymbols` input.
