...
const reviewerSchema = `Validate the supplied result directly. Repository tools are a fallback only when a specific claim cannot be checked from the prompt; use at most 4 tool calls. Return {"schemaVersion":1,"verdict":"approved"|"request_changes","summary":string,"findings":[{"severity":"blocking"|"non_blocking","message":string,"file"?:string,"line"?:number}]}.`;
...
const systems: Record<Role, string> = {
  ...
  documentation: "You analyze and implement documentation impact. Keep changes limited to documentation. Explicitly justify creating, updating, or omitting README, files under docs/ or prds/, and AGENTS.md.",
...
export class Workflow {
  ...
  private async repairValidation(directory: string, manifest: Manifest, report: ValidationReport): Promise<ValidationReport> {
    const history = path.join(directory, "history");
    while (!report.passed) {
      if (manifest.iterations.implementation >= this.config.limits.implementationReviewIterations) throw new Error("Deterministic validation repair limit exhausted");
      await moveState(directory, manifest, "implementation_fix");
      const fixed = await this.call(directory, manifest, "implementer", `Fix only the deterministic validation failures shown below. Preserve the approved plan and avoid unrelated changes. Return {"summary":string,"resolutions":string[]}.\n\n${report.summary}`, manifest.worktree!, true);
      manifest.iterations.implementation += 1; await writeArtifact(history, `implementation/iteration-${String(manifest.iterations.implementation).padStart(3, "0")}.md`, String((fixed.structured as { summary?: unknown }).summary ?? "Validation failures addressed."), this.config); await this.snapshot(history, manifest.worktree!, manifest.iterations.implementation); await moveState(directory, manifest, "deterministic_validation"); report = await this.validation(directory, manifest, manifest.iterations.implementation);
    }
    return report;
  }
  private async reviewLoops(directory: string, manifest: Manifest, requirements: string, plan: string): Promise<void> {
    const history = path.join(directory, "history"); let report = await this.validation(directory, manifest, manifest.iterations.implementation); report = await this.repairValidation(directory, manifest, report); await moveState(directory, manifest, "code_review");
